 ...
 *
 * Handles Nagad payment creation and callback endpoints.
 */
class PaymentController extends Controller
{
    ...
    public function create(Request $request, PaymentManager $paymentManager)
    {
        ...
        // Extract product_id and invoice, generate invoice if not provided
        ...
        try {
            // Initialize Nagad payment session
            $initialize = $paymentManager->nagad()->initializePayment($invoice);

            // Prepare payload for payment execution
            $payload = [
                'user_id' => $user_id,
                'invoice' => $invoice,
                'amount'  => $amount,
